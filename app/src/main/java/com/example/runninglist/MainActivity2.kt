...
class MainActivity2 : AppCompatActivity(),AboutTaskAdapter.Listener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AddTaskLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
            ...
            if(it.resultCode == RESULT_OK){
                ...
                var OsnTask = AboutTask(BigRandom,task.NameTask,task.ImageTask1,task.ImageTask2,task.ImageTask3, task.ImageTask4,task.ImageTask5,task.ImageTask6,task.ImageTask7,task.Day,task.Description);
        ...
        binding.apply {
            ...
            // тут  функцию getAllTaskUser и обновления списка
            //GetUserTask()
            ...
            menuchka.setNavigationItemSelectedListener {
                when(it.itemId){
                    ...
                    AddTaskLauncher!!.launch(Intent(this@MainActivity2,AddTaskScrean::class.java))
                    }
                   R.id.backscreen ->{
                ...
                if (cod == 7) {
                    ...
                    if (isNumeric(codif) == true) {
                        ...
                        if(ContainD(codif) == true){
                            Toast.makeText(this,"Ежедневные задачи нельзя передвинуть",Toast.LENGTH_SHORT).show()
                        }
                        else {
                        when (codif) {
                            "12i" -> {
    ...
    private fun GetUserTask(){
        lifecycleScope.launch(Dispatchers.IO) {
        var tasks =  (applicationContext as App).repository.getAllTask()
